...
export async function createProduct(productData: IProduct) {
    ...
    if (nameProductExist) throw new ValidationError('Já existe um produto com esse nome.');
...
//Editar produto
export async function editProduct(productData: IEditProduct) {
    const { description, isAvailable, name, price, id } = productData;
    if (!description || !isAvailable || !name || !price || !id) throw new ValidationError('Campos não podem ficar vazios');
